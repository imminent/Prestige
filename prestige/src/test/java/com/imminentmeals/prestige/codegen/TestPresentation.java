...
public class TestPresentation {
  ...
  private static final String _IMPORT_PRESENTATION =
      "import com.imminentmeals.prestige.annotations.Presentation;";
  private static final String _PRESENTATION_ANNOTATION = "@Presentation";
  private static final String _TEST_CLASS = "Test";

  @Test public void testPresentation() {
    final JavaFileObject presentation_interface =
  ...
  @Test public void testPresentationFailsIfNotInterface() {
    final JavaFileObject presentation =
        JavaFileObjects.forSourceString("NotInterfacePresentation", Joiner.on('\n').join(
              _IMPORT_PRESENTATION
            , _PRESENTATION_ANNOTATION
    ...
    ASSERT.about(javaSource())
        ...
        .failsToCompile()
        .withErrorContaining(
            String.format("@Presentation annotation may only be specified on interfaces (%s)."
                , "NotInterfacePresentation"))
        .in(presentation)
        .onLine(THREE);
  }

  @Test public void testPresentationFailsIfNotPublic() {
    JavaFileObject presentation =
        JavaFileObjects.forSourceString("PackageProtectedPresentation", Joiner.on('\n').join(
              _IMPORT_PRESENTATION
            , _PRESENTATION_ANNOTATION
    ...
    presentation = JavaFileObjects.forSourceString(_TEST_CLASS, Joiner.on('\n').join(
          _IMPORT_PRESENTATION
        , "public class Test {"
        , _PRESENTATION_ANNOTATION
  ...
  @Test public void testProtocolFailsIfNotInterface() {
    ...
    final JavaFileObject presentation =
        JavaFileObjects.forSourceString("PresentationWithProtocolInterface_broken"
            , Joiner.on('\n').join(
              _IMPORT_PRESENTATION
  ...
  @Test public void testProtocolFailsIfNotPublic() {
    ...
    JavaFileObject source =
        JavaFileObjects.forSourceString(_TEST_CLASS, String.format(source_format, "", protocol_name));
  ...
  @Test public void testProtocolFailsIfNotExtendingPresentationFragmentProtocols() {
    final JavaFileObject presentation_fragment_protocol =
