...
import com.imminentmeals.prestige.Finder;
...
import static com.imminentmeals.prestige.codegen.utilities.CaseFormat.LOWER_CAMEL;
...
/* package */ class CodeGenerator {
  ...
  public void generateSourceCode(List<PresentationControllerBinding> controllers,
      List<ModuleData> controller_modules,
    ...
    try {
      ...
      writer.flush();
      generateSegueControllerSourceCode(writer, controllers, controller_modules, model_modules,
          model_interfaces
      ...
      for (DataSourceInjectionData data_source_injection : data_source_injections) {
        ...
        source_code =
            _filer.createSourceFile(_elements.getBinaryName(element) + DATA_SOURCE_INJECTOR_SUFFIX,
                element);
        writer = source_code.openWriter();
        writer.flush();
        generateDataSourceInjector(writer, data_source_injection.package_name,
            data_source_injection.target,
      ...
      for (ModuleData model_module : model_modules) {
        ...
        writer = source_code.openWriter();
        writer.flush();
        generateModelModule(writer, model_module.package_name,
            (List<ModelData>) model_module.components,
      ...
      for (Map.Entry<Element, Map<Integer, List<PresentationFragmentInjectionData>>> injection : presentation_fragment_injections
          .entrySet()) {
        final TypeElement element = (TypeElement) injection.getKey();
        final String full_name =
            _elements.getBinaryName(element) + PRESENTATION_FRAGMENT_INJECTOR_SUFFIX;
        source_code = _filer.createSourceFile(full_name, element);
        writer = source_code.openWriter();
        writer.flush();
        final String package_name = _elements.getPackageOf(element) + "";
        final String class_name = full_name.substring(package_name.length() + 1);
        generatePresentationFragmentInjector(writer, package_name, injection.getValue(), class_name,
            element);
      }
      for (Map.Entry<Element, List<PresentationFragmentInjectionData>> injection : controller_presentation_fragment_injections
          .entrySet()) {
        final TypeElement element = (TypeElement) injection.getKey();
        final String full_name =
            _elements.getBinaryName(element) + PRESENTATION_FRAGMENT_INJECTOR_SUFFIX;
        ...
        final String package_name = _elements.getPackageOf(element) + "";
        final String class_name = full_name.substring(package_name.length() + 1);
        generateControllerPresentationFragmentInjector(writer, package_name, injection.getValue(),
            class_name, element);
      }

      // Generates the $$PresentationInjectors
      for (Map.Entry<Element, PresentationInjectionData> injection : controller_presentation_injections
          .entrySet()) {
        final TypeElement element = (TypeElement) injection.getKey();
        ...
        final String package_name = _elements.getPackageOf(element) + "";
        final String class_name = full_name.substring(package_name.length() + 1);
        generateControllerPresentationInjector(writer, package_name, injection.getValue(),
            class_name, element);
      }
   ...
   * @throws IOException
   */
  private void generateSegueControllerSourceCode(Writer writer,
      List<PresentationControllerBinding> controllers,
    ...
    java_writer.emitSingleLineComment(_HEADER_COMMENT)
        ...
        .emitImports(JavaWriter.type(Map.class)
    ...
    for (PresentationControllerBinding binding : controllers) {
      java_writer.emitJavadoc("Provider for instances of the {@link %s} Controller",
          binding.controller)
    ...
    emitStoreMethod(java_writer, models, element_to_model_interfaces, model_implementations,
        public_modifier, else_if);
    // createObjectGraph
    ...
    if (!controller_modules.isEmpty()) {
      ...
      for (ModuleData controller_module : controller_modules)
        if (controller_module.scope.equals(Implementations.PRODUCTION)) {
          production_module =
              String.format(Locale.US, "modules.add(new %s())", controller_module.qualified_name);
        } else {
    ...
    if (!model_modules.isEmpty()) {
      ...
      for (ModuleData model_module : model_modules)
        if (model_module.scope.equals(Implementations.PRODUCTION)) {
          production_module =
              String.format(Locale.US, "modules.add(new %s(this))", model_module.qualified_name);
        } else {
          java_writer.beginControlFlow(String.format(Locale.US, "%s (scope.equals(\"%s\"))"
    ...
    java_writer.emitEmptyLine()
        .emitAnnotation(Override.class)
        .emitAnnotation(Nonnull.class)
        .beginMethod(JavaWriter.type(Map.class, JavaWriter.type(Class.class, "?"),
            JavaWriter.type(Provider.class))
            , "bindPresentationsToControllers", protected_modifier)
        .emitStatement("final Map<Class<?> Provider> controllers = new HashMap<>()")
        .emitStatement("controllers%n%s", controller_puts)
        .emitStatement("return controllers")
        .endMethod();
    ...
    java_writer.emitEmptyLine()
        ...
        .emitStatement("final Map<Class<?>, Lazy> models = new HashMap<>()")
        .emitStatement("models%n%s", model_puts)
        .emitStatement("return models")
        .endMethod();
    java_writer.endType()
        .emitEmptyLine();
    java_writer.close();
  ...
  private static StringBuilder emitModelFields(JavaWriter java_writer, List<ModelData> models,
      StringBuilder model_puts)
      throws IOException {
    for (ModelData model : models) {
      ...
      if (model.should_serialize) {
        java_writer.emitJavadoc("Provider for {@link %s} for {@link %s}",
            JavaWriter.type(GsonConverter.class)
            , model.contract)
            .emitAnnotation(Inject.class)
            ...
            .emitField(JavaWriter.type(Lazy.class, JavaWriter.type(GsonConverter.class))
                , model.variable_name + _CONVERTER);
      }
    }
    return model_puts;
  }

  private void emitStoreMethod(JavaWriter java_writer, List<ModelData> models,
      Map<Element, ModelData> element_to_model_interfaces
      , Map<String, Map<Element, ModelData>> model_implementations,
      EnumSet<Modifier> public_modifier
      , String else_if) throws IOException {
    String if_else_if_control;
    java_writer.emitEmptyLine()
        .emitAnnotation(Override.class)
        .beginMethod("<T> void", "store", public_modifier, Arrays.asList("T", "obbject")
            , Arrays.asList(JavaWriter.type(IOException.class)));
    if_else_if_control = "if";
    ...
    for (ModelData model : models) {
      ...
      java_writer.beginControlFlow(
          String.format("%s (object instanceof %s)", if_else_if_control, model.contract))
          .emitStatement("Timber.d(\"Storing \" + object)")
          .emitStatement(
              "%s%s.get().toStream(object, gson_provider.get().outputStreamFor(%s.class))"
              , model.variable_name, _CONVERTER, model.contract);
      ...
      for (Map.Entry<String, Map<Element, ModelData>> entry : model_implementations.entrySet()) {
        final ModelData model_implementation =
            modelImplementation(model, model_implementations, entry.getValue());
        if (shouldSkipModelImplementation(model_implementation)) continue;

        java_writer.beginControlFlow(
            String.format("%s (scope.equals(\"%s\"))", nested_if_else_if, entry.getKey()));
  ...
  private void emitRecursiveStoreCalls(JavaWriter java_writer,
      Map<Element, ModelData> element_to_model_interfaces, ModelData model_implementation)
      throws IOException {
    for (Element parameter : model_implementation.parameters) {
      final ModelData sub_model =
          element_to_model_interfaces.get(_types.asElement(parameter.asType()));
      if (!sub_model.should_serialize) continue;
      java_writer.emitStatement("store(createModel(%s.class))", sub_model.contract);
    }
  }

  private static ModelData modelImplementation(ModelData model,
      Map<String, Map<Element, ModelData>> model_implementations,
   ...
   */
  private void generateControllerModule(Writer writer, String package_name,
      List<ControllerData> controllers,
    ...
    attributes.put("injects", "{\n" +
        "_SegueController.class" +
        "\n}");
    attributes.put("overrides", !class_name.equals(_DEFAULT_CONTROLLER_MODULE));
    attributes.put("library", true);
    attributes.put("complete", false);
    java_writer.emitJavadoc("<p>Module for injecting:\n" +
        "<ul>\n" +
        "%s" +
        "</ul></p>", controller_list)
        .emitAnnotation(Module.class, attributes)
        ...
        .beginMethod("com.squareup.otto.Bus", "providesControllerBus",
   ...
   * @param class_name
   */
  private void generateDataSourceInjector(Writer writer, String package_name, Element target,
  ...
  private void generateModelModule(Writer writer, String package_name, List<ModelData> models,
    ...
    java_writer.emitSingleLineComment(_HEADER_COMMENT)
        ...
        .emitImports(_SEGUE_CONTROLLER_CLASS,
            ...
            JavaWriter.type(SegueController.class),
            JavaWriter.type(Timber.class),
            JavaWriter.type(HashSet.class),
            JavaWriter.type(Arrays.class));
    if (has_default_models && is_default_module) {
      java_writer.emitStaticImports(default_model_module + ".default_models");
    }
    java_writer.emitEmptyLine();
    final Map<String, Object> attributes = new HashMap<>(5);
    attributes.put("inject", "{\n" +
        "_SegueController.class" +
        "\n}");
    attributes.put("overrides", is_default_module);
    attributes.put("library", true);
    attributes.put("complete", false);
    java_writer.emitJavadoc("<p>Module for injecting:\n" +
        "<ul>\n" +
    ...
    final List<String> model_exclusions = new ArrayList<>(models.size());
    ...
    if (class_name.equals(_DEFAULT_MODEL_MODULE)) {
      ...
      final String put = "default_models.put(%s)";
      for (String exclusion : model_exclusions)
        java_writer.emitStatement(put, exclusion);
    }
    java_writer.emitEmptyLine()
    ...
    for (ModelData model : models) {
      ...
      String[] provider_parameters = model.should_serialize
          ...
          JavaWriter.type(GsonProvider.class)
          , "gson_provider"
          , "@Named(" + stringLiteral(model.contract + "") + ") " + JavaWriter.type(
          GsonConverter.class)
          , "converter"
          ...
          , "controller_bus"}
          : new String[] {
              "@Named(com.imminentmeals.prestige.ControllerContract.BUS) " + JavaWriter.type(
                  Bus.class)
              , "controller_bus"
          };
      ...
      if (model.parameters == null || model.parameters.isEmpty()) {
        ...
        final Set<String> provider_method_parameters =
            new LinkedHashSet<>(Arrays.asList(provider_parameters));
        final List<String> constructor_parameters =
            constructorParameters(model, provider_method_parameters);
      ...
      if (model.should_serialize) {
        java_writer.emitStatement("InputStream input_stream = null")
            ...
            .emitStatement("Timber.d(\"Restoring %s from input stream\")",
                java_writer.compressType(model.implementation
                    + ""))
            .emitStatement("model = (%s) converter.from(input_stream)",
                java_writer.compressType(model.implementation
                    + ""))
        ...
        java_writer.emitEmptyLine()
            ...
            .emitStatement("return new %s(gson, %s.class)"
                , JavaWriter.type(GsonConverter.class, model.implementation + ""),
    ...
    for (ModelData model : models) {
      ...
      if (model.should_serialize) {
        java_writer.emitStatement(
            "final InstanceCreator<%1$s> %1$s_creator = _segue_controller.instanceCreator(%2$s.class)"
    ...
    java_writer.emitStatement(new_line_joiner.join(
        ...
        ? "default_models;"
        : (has_default_models ? "default_models.addAll(Arrays.asList(%s));"
        ...
        .emitField(JavaWriter.type(String.class), "_TAG", EnumSet.of(PRIVATE, STATIC, FINAL),
            stringLiteral("Prestige"))
        .emitJavadoc("Segue Controller")
        .emitField(JavaWriter.type(SegueController.class), "_segue_controller", private_final);
    for (ModelData model : models)
      if (model.should_serialize) {
        java_writer.emitField(JavaWriter.type(boolean.class),
            '_' + model.variable_name + file_tested, private_modifier, "false");
      }
    java_writer.endType();
    java_writer.close();
  }

  private List<String> constructorParameters(ModelData model,
      Set<String> provider_method_parameters) {
    final List<String> constructor_parameters = new ArrayList<>();
   ...
   * @param class_name
   */
  private void generateModelInjector(Writer writer, String package_name,
      List<ModelInjectionData> injections,
    ...
    java_writer.emitSingleLineComment(_HEADER_COMMENT)
        .emitPackage(package_name)
        .emitEmptyLine()
        .emitJavadoc("<p>Injects the Models into {@link %s} and stores them for later use.</p>",
            class_name)
        .beginType(class_name, _CLASS, EnumSet.of(PUBLIC, FINAL))
        .emitEmptyLine()
    ...
    for (ModelInjectionData injection : injections)
      java_writer.emitStatement("target.%s = segue_controller.createModel(%s.class)",
    ...
    java_writer.endMethod()
        ...
        .beginMethod(_VOID, "storeModels", EnumSet.of(PUBLIC, STATIC),
            Arrays.asList(JavaWriter.type(SegueController.class), _SEGUE_CONTROLLER,
  ...
  private void generateControllerPresentationFragmentInjector(Writer writer, String package_name,
      List<PresentationFragmentInjectionData> injections, String class_name, Element target)
      throws IOException {
    final JavaWriter java_writer = new JavaWriter(writer);
    java_writer.setCompressingTypes(true);
    java_writer.emitSingleLineComment(_HEADER_COMMENT)
    ...
    if (!injections.isEmpty()) {
      ...
      java_writer.beginControlFlow(
          String.format(control_format, "if", injection.implementation, injection.tag))
          .emitStatement("target.%s = (%s) presentation_fragment", injection.variable_name,
              injection.variable.asType())
          .endControlFlow();
    }
    for (PresentationFragmentInjectionData injection : injections.subList(min(1, injections.size()),
        injections.size()))
      java_writer.beginControlFlow(
          String.format(control_format, "else if", injection.implementation, injection.tag))
          .emitStatement("target.%s = (%s) presentation_fragment", injection.variable_name,
              injection.variable.asType())
          .endControlFlow();
  ...
  private void generatePresentationFragmentInjector(Writer writer, String package_name,
      Map<Integer, List<PresentationFragmentInjectionData>> injections, String class_name,
      Element target) throws IOException {
    final JavaWriter java_writer = new JavaWriter(writer);
    java_writer.setCompressingTypes(true);
    java_writer.emitSingleLineComment(_HEADER_COMMENT)
        .emitPackage(package_name)
    ...
    if (!injections.isEmpty()) {
      java_writer.emitStatement("final Context context = finder.findContext(target)")
          .emitStatement(
              "final FragmentManager fragment_manager = finder.findFragmentManager(target)")
      ...
      for (Map.Entry<Integer, List<PresentationFragmentInjectionData>> entry : injections.entrySet()) {
        ...
        for (PresentationFragmentInjectionData injection : entry.getValue()) {
          java_writer.emitStatement("target.%s = (%s) Fragment.instantiate(context, \"%s\")",
              injection.variable_name,
  ...
  private void generateControllerPresentationInjector(Writer writer, String package_name,
      PresentationInjectionData injection,
      String class_name, Element target) throws IOException {
    final JavaWriter java_writer = new JavaWriter(writer);
    java_writer.setCompressingTypes(true);
    java_writer.emitSingleLineComment(_HEADER_COMMENT)
    ...
    java_writer.beginControlFlow(
        String.format(Locale.US, control_format, injection.variable.asType()))
  ...
  private static final String _CONVERTER = "_converter";
  private static final String _DEFAULT_CONTROLLER_MODULE =
      LOWER_CAMEL.to(UPPER_CAMEL, PRODUCTION) + CONTROLLER_MODULE_SUFFIX;
  private static final String _DEFAULT_MODEL_MODULE =
      LOWER_CAMEL.to(UPPER_CAMEL, PRODUCTION) + MODEL_MODULE_SUFFIX;
  private static final String _CLASS = "class";
  private static final String _SEGUE_CONTROLLER_CLASS =
  ...
  private static final String _HEADER_COMMENT =
      "Generated code from Prestige. Modification is futile.";
}
