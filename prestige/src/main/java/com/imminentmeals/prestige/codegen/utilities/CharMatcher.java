...
import static com.imminentmeals.prestige.codegen.utilities.Preconditions.checkArgument;
 ...
 *
 * <p>See the Guava User Guide article on <a href= "http://code.google.com/p/guava-libraries/wiki/StringsExplained#CharMatcher">
 * {@code CharMatcher}</a>.
 *
 * @author Kevin Bourrillion
 * @since 1.0
 */
@SuppressWarnings("UnusedDeclaration") public abstract class CharMatcher {
  // Constants
  /**
  ...
  public static final CharMatcher DIGIT =
      new RangesMatcher("CharMatcher.DIGIT", ZEROES.toCharArray(), NINES.toCharArray());
  ...
  private static class NegatedMatcher extends CharMatcher {
    ...
    @Override CharMatcher withToString(String description) {
  ...
  /**
   * Subclasses should provide a new CharMatcher with the same characteristics as {@code this}, but
   * with their {@code toString} method overridden with the new description.
   *
  ...
  abstract static class FastMatcher extends CharMatcher {
    ...
    @Override public final CharMatcher precomputed() {
      return this;
    }
  ...
  private static class BitSetMatcher extends FastMatcher {
    ...
    @Override void setBits(BitSet bitSet) {
      bitSet.or(table);
    }
  }
  ...
  /**
   * A special-case CharMatcher for Unicode whitespace characters that is extremely efficient both
   * in space required and in time to check for matches.
   *
   * Implementation details. It turns out that all current (early 2012) Unicode characters are
   * unique modulo 79: so we can construct a lookup table of exactly 79 entries, and just check the
   * character code mod 79, and see if that character is in the table.
   *
   * There is a 1 at the beginning of the table so that the null character is not listed as
   * whitespace.
   *
   * Other things we tried that did not prove to be beneficial, mostly due to speed concerns:
   *
   * * Binary search into the sorted list of characters, i.e., what CharMatcher.anyOf() does</li> *
   * Perfect hash function into a table of size 26 (using an offset table and a special Jenkins hash
   * function)</li> * Perfect-ish hash function that required two lookups into a single table of
   * size 26.</li> * Using a power-of-2 sized hash table (size 64) with linear probing.</li>
   *
