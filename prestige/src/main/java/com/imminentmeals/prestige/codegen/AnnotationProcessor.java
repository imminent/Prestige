...
import com.imminentmeals.prestige.codegen.utilities.CaseFormat;
import com.imminentmeals.prestige.codegen.utilities.Joiner;
...
import static com.imminentmeals.prestige.codegen.utilities.Utilities.getAnnotation;
...
public class AnnotationProcessor extends AbstractProcessor {
  ...
  public static final String PRESENTATION_FRAGMENT_INJECTOR_SUFFIX =
      "$$PresentationFragmentInjector";
  ...
  public boolean process(Set<? extends TypeElement> _, RoundEnvironment environment) {
    ...
    final List<PresentationControllerBinding> presentation_controller_bindings = new ArrayList<>();
    final Map<String, List<ControllerData>> controllers = new HashMap<>();
    final List<DataSourceInjectionData> data_source_injections = new ArrayList<>();
    final Map<String, List<ModelData>> models = new HashMap<>();
    final List<ModelData> model_interfaces = new ArrayList<>();
    final Map<Element, List<ModelInjectionData>> model_injections = new HashMap<>();
    final Map<Element, List<PresentationFragmentInjectionData>> presentation_fragment_injections =
        new HashMap<>();
    final Map<Element, List<PresentationFragmentInjectionData>>
        controller_presentation_fragment_injections = new HashMap<>();
    final Map<Element, PresentationInjectionData> controller_presentation_injections =
        new HashMap<>();

    // Processes the @PresentationFragment and @PresentationFragmentImplementation annotations
    final Map<Element, PresentationFragmentData> presentation_fragments =
        processPresentationFragments(environment);

    // Processes the @Presentation and @PresentationImplementation annotations
    final Map<Element, PresentationData> presentations =
        processPresentations(environment, presentation_fragments);
    ...
    final List<ModuleData> controller_modules = new ArrayList<>(controllers.size());
    for (Entry<String, List<ControllerData>> controller_implementations : controllers.entrySet()) {
      final Element implementation = controller_implementations.getValue().get(0).implementation;
      final String package_name =
          _element_utilities.getPackageOf(implementation).getQualifiedName() + "";
    ...
    final List<ModuleData> model_modules = new ArrayList<>();
    for (Entry<String, List<ModelData>> model_implementations : models.entrySet()) {
    ...
    final Map<Element, Map<Integer, List<PresentationFragmentInjectionData>>>
        presentation_fragment_display_injections =
        new HashMap<>(presentation_fragment_injections.size());
    for (Entry<Element, List<PresentationFragmentInjectionData>> entry : presentation_fragment_injections
        .entrySet()) {
      final Map<Integer, List<PresentationFragmentInjectionData>> injections = new HashMap<>();
      final List<PresentationFragmentInjectionData> presentation_fragment_list = entry.getValue();
      if (presentation_fragment_list != null) {
        for (PresentationFragmentInjectionData presentation_fragment : presentation_fragment_list) {
          ...
          for (Entry<Integer, Integer> injection : presentation_fragment.displays.entrySet())
            if (injections.containsKey(injection.getKey())) {
              ...
              final List<PresentationFragmentInjectionData> injection_list =
                  new ArrayList<>(presentation_fragment.displays.size());
              injection_list.add(presentation_fragment);
              injections.put(injection.getKey(), injection_list);
            }
        }
      }
      presentation_fragment_display_injections.put(entry.getKey(), injections);
    }
    final Map<Element, ModelData> element_to_model_map = new HashMap<>(model_interfaces.size());
    for (ModelData model : model_interfaces) element_to_model_map.put(model.contract, model);
    final Map<String, Map<Element, ModelData>> model_implementations = new HashMap<>();
    ...
    for (Entry<String, List<ModelData>> entry : models.entrySet()) {
      ...
      if (scoped_models == null) {
        scoped_models = new HashMap<>(number_of_scopes);
    ...
    generator.generateSourceCode(presentation_controller_bindings, controller_modules,
        data_source_injections,
        model_modules, model_interfaces, element_to_model_map,
        model_implementations, model_injections,
        presentation_fragment_display_injections,
  ...
  /**
   * <p>Processes the source code for the @PresentationFragment and @PresentationFragmentImplementation
   * annotations.</p>
   *
   * @param environment The round environment
   */
  private Map<Element, PresentationFragmentData> processPresentationFragments(
      RoundEnvironment environment) {
    final Map<Element, Element> presentation_fragment_protocols = new HashMap<>();
    final Map<Element, Element> presentation_fragment_implementations = new HashMap<>();
    final Map<Element, Set<Element>> unverified_presentation_fragments = new HashMap<>();
    final TypeMirror no_protocol =
        _element_utilities.getTypeElement(PresentationFragment.NoProtocol.class.getCanonicalName())
            .asType();
    ...
    for (Element element : environment.getElementsAnnotatedWith(PresentationFragment.class)) {
      ...
      if (verifyPublicInterface(element
          ...
          , "@PresentationFragment interfaces must be public (%s).")) {
        continue;
      }

      // Gathers @PresentationFragment information
      final Map<String, Object> parsed_annotation =
          getAnnotation(PresentationFragment.class, element);
      ...
      if (verifyPublicInterface(protocol
          ...
          , "@PresentationFragment Protocol must be public (%s).")) {
        continue;
      }
      ...
      if (!_type_utilities.isSameType(protocol.asType(), no_protocol)
          && unverified_presentation_fragments.containsKey(element)) {
        for (Element presentation_fragment : unverified_presentation_fragments.get(element)) {
          final TypeMirror super_protocol =
              presentation_fragment_protocols.get(presentation_fragment).asType();
    ...
    final Map<Element, PresentationFragmentData> result =
        new HashMap<>(presentation_fragment_protocols.size());
    for (Entry<Element, Element> entry : presentation_fragment_protocols.entrySet()) {
      final Element presentation_fragment = entry.getKey();
      final Element protocol = entry.getValue();
      result.put(entry.getKey(), new PresentationFragmentData(
          protocol == null ? _type_utilities.asElement(no_protocol) : protocol
          , presentation_fragment_implementations.get(presentation_fragment)));
    }
    return result;
  }
  ...
  private void verifyPresentationFragments(Map<Element, Element> presentation_fragment_protocols,
  ...
  private void processPresentationFragmentImplementations(RoundEnvironment environment
    ...
    final Set<Element> presentation_fragments =
        new HashSet<>(presentation_fragment_protocols.keySet());
    for (Element implementation_element : environment.getElementsAnnotatedWith(
        PresentationFragmentImplementation.class)) {
  ...
  private void verifyPresentationFragmentSubProtocols(
      Map<Element, Element> presentation_fragment_protocols
      , Map<Element, Set<Element>> unverified_presentation_fragments
    ...
    for (Element enclosed_element : implementation_element.getEnclosedElements()) {
      if (enclosed_element.getAnnotation(InjectPresentationFragment.class) != null &&
        ...
        final Element sub_presentation_fragment =
            _type_utilities.asElement(enclosed_element.asType());
        note("\tcontains Presentation Fragment: " + sub_presentation_fragment);
        if (!_type_utilities.isSameType(
            presentation_fragment_protocols.get(sub_presentation_fragment).asType(), no_protocol)) {
          ...
          final Set<Element> presentation_fragment_list = new HashSet<>();
          presentation_fragment_list.add(presentation_fragment);
          unverified_presentation_fragments.put(enclosed_element, presentation_fragment_list);
        }
      }
    }
  }
  ...
  /**
   * <p>Processes the source code for the @Presentation and @PresentationImplementation
   * annotations.</p>
   *
   * @param environment The round environment
   */
  private Map<Element, PresentationData> processPresentations(RoundEnvironment environment,
      Map<Element, PresentationFragmentData> presentation_fragments) {
    final Map<Element, Element> presentation_protocols = new HashMap<>();
    final Map<Element, Element> presentation_implementations = new HashMap<>();
    ...
    for (Element element : environment.getElementsAnnotatedWith(Presentation.class)) {
      ...
      if (verifyPublicInterface(element
          ...
          , "@Presentation interface must be public (%s).")) {
        continue;
      }
      ...
      if (verifyPublicInterface(protocol
          ...
          , "@Presentation Protocol must be public (%s).")) {
        continue;
      }
    ...
    processPresentationImplementations(environment, presentation_fragments, presentation_protocols,
        presentation_implementations);

    // TODO: Should require there to exist a @PresentationImplementation for @Controller to work?? Good?
    final Map<Element, PresentationData> result = new HashMap<>(presentation_protocols.size());
    for (Entry<Element, Element> presentation_protocol : presentation_protocols.entrySet()) {
      final Element presentation = presentation_protocol.getKey();
      final Element protocol = presentation_protocol.getValue();
      result.put(presentation,
          new PresentationData(protocol, presentation_implementations.get(presentation)));
    }
    return result;
  }
  ...
  private void processPresentationImplementations(RoundEnvironment environment
    ...
    final Set<Element> presentations = new HashSet<>(presentation_protocols.keySet());
    for (Element implementation_element : environment.getElementsAnnotatedWith(
        PresentationImplementation.class)) {
      ...
      // Finds Presentation Fragment injections and verifies that their Protocols are met
      verifyPresentationSubProtocols(presentation_fragments,
          presentation_protocols.get(presentation).asType()
  ...
  private void verifyPresentationSubProtocols(
      Map<Element, PresentationFragmentData> presentation_fragments, TypeMirror protocol
    ...
    for (Element enclosed_element : implementation_element.getEnclosedElements()) {
      if (enclosed_element.getAnnotation(InjectPresentationFragment.class) != null &&
        ...
        if (presentation_fragment_protocol != null &&
            !_type_utilities.isSameType(presentation_fragment_no_protocol,
                presentation_fragment_protocol.asType()) &&
  ...
  private void processDataSourceInjections(RoundEnvironment environment,
      ...
      Map<Element, PresentationData> presentations,
      Map<Element, PresentationFragmentData> presentation_fragments) {
    final TypeMirror no_protocol =
        _element_utilities.getTypeElement(NoProtocol.class.getCanonicalName()).asType();
    final TypeMirror presentation_fragment_no_protocol = _element_utilities.getTypeElement(
        PresentationFragment.NoProtocol.class.getCanonicalName()).asType();
    ...
    for (Element element : environment.getElementsAnnotatedWith(InjectDataSource.class)) {
      ...
      final boolean is_presentation =
          enclosing_element.getAnnotation(PresentationFragmentImplementation.class) == null;
      final TypeMirror element_no_protocol =
          is_presentation ? no_protocol : presentation_fragment_no_protocol;
      ...
      if (verifyDataSourceProtocol(element, enclosing_element, is_presentation, element_no_protocol,
          protocol)) {
        continue;
      }
      ...
      data_source_injections.add(
          new DataSourceInjectionData(package_name, enclosing_element, element, element_class));
    }
  }

  private boolean verifyDataSourceProtocol(Element element, TypeElement enclosing_element,
      boolean is_presentation
    ...
    if (!_type_utilities.isSameType(element.asType(), protocol)) {
      error(element, "@InjectDataSource fields must be the same as the " +
          ...
          " Protocol, which is %s (%s.%s).", protocol, enclosing_element.getQualifiedName(),
          element);
      // Skips the current element
      return true;
    }
  ...
  private TypeMirror expectedDataSourceProtocol(Map<Element, PresentationData> presentations
      , Map<Element, PresentationFragmentData> presentation_fragments
  ...
  /**
   * <p>Processes the source code for the @Controller and @ControllerImplementation
   * annotations.</p>
   *
   * @param environment The round environment
   * @param presentation_controller_bindings Will hold the bindings between Presentation
   * implementations and their Controllers
  ...
  private void processControllers(RoundEnvironment environment,
      ...
      Map<Element, PresentationData> presentations) {
    final TypeMirror default_presentation =
        _element_utilities.getTypeElement(Default.class.getCanonicalName()).asType();
    final TypeMirror no_protocol =
        _element_utilities.getTypeElement(NoProtocol.class.getCanonicalName()).asType();
    final Set<Element> controller_interfaces = new HashSet<>();
    ...
    for (Element element : environment.getElementsAnnotatedWith(Controller.class)) {
      ...
      if (verifyPublicInterface(element
          ...
          , "@Controller interface must be public (%s).")) {
        continue;
      }
  ...
  private void processControllerImplementations(RoundEnvironment environment
    ...
    for (Element implementation_element : environment.getElementsAnnotatedWith(
        ControllerImplementation.class)) {
      ...
      final String scope =
          implementation_element.getAnnotation(ControllerImplementation.class).value();
      ...
      if (implementations == null) {
        final List<ControllerData> controller_list = new ArrayList<>();
        controller_list.add(new ControllerData(controller, implementation_element));
        controllers.put(scope, controller_list);
        // Verifies that the scope-grouped @ControllerImplementations are in the same package
      } else if (!_element_utilities.getPackageOf(implementations.get(0).implementation)
          .equals(package_name)) {
        error(controller,
  ...
  private Element presentationForController(Map<Element, PresentationData> presentations
    ...
    if (_type_utilities.isSameType(presentation.asType(), default_presentation)) {
      ...
      for (Element presentation_interface : presentations.keySet())
        if (presentation_interface.getSimpleName()
            .contentEquals(presentation_from_controller_name)) {
      ...
      if (_type_utilities.isSameType(presentation.asType(), default_presentation)) {
        error(element, "No @Presentation-annotated %s found, implicitly required by %s (in %s)"
            , presentation_from_controller_name, element,
  ...
  private void processModels(RoundEnvironment environment, Map<String, List<ModelData>> models,
    ...
    final TypeElement gson_provider =
        _element_utilities.getTypeElement(GsonProvider.class.getName());
    final Set<Element> elements = (Set<Element>) environment.getElementsAnnotatedWith(Model.class);
    final Map<Element, Boolean> model_should_be_serialized = new HashMap<>();
    ...
    for (Element element : elements) {
      ...
      if (verifyPublicInterface(element
          ...
          , "@Model interface must be public (%s).")) {
        continue;
      }
  ...
  private void processModelImplementations(RoundEnvironment environment,
      Map<String, List<ModelData>> models, Map<Element, Boolean> model_should_be_serialized) {
    final Set<Element> model_interfaces = new HashSet<>(model_should_be_serialized.keySet());
    for (Element implementation_element : environment.getElementsAnnotatedWith(
      ...
      final ModelImplementation model_implementation =
          implementation_element.getAnnotation(ModelImplementation.class);
      ...
      final List<? extends VariableElement> parameters =
          injectModelConstructor((TypeElement) implementation_element);
      if (implementations == null) {
        final List<ModelData> model_list = new ArrayList<>();
        model_list.add(new ModelData(model, implementation_element, parameters, should_serialize));
        models.put(scope, model_list);
        // Verifies that the scope-grouped @ControllerImplementations are in the same package
      } else if (!_element_utilities.getPackageOf(implementations.get(0).implementation)
          .equals(package_name)) {
        ...
        implementations.add(
            new ModelData(model, implementation_element, parameters, should_serialize));
      }
      ...
      if (!should_serialize && model_should_be_serialized.get(model)) {
        processingEnv.getMessager().printMessage(WARNING
            , String.format("Should make all implementations of %s serialized",
  ...
  private void processModelInjections(RoundEnvironment environment,
      Map<Element, List<ModelInjectionData>> model_injections,
      List<ModelData> models) {
    final Map<Element, Boolean> model_interfaces = new HashMap<>(models.size());
    ...
    for (Element element : environment.getElementsAnnotatedWith(InjectModel.class)) {
      ...
      final String package_name = _element_utilities.getPackageOf(enclosing_element) + "";
      final String element_class = _element_utilities.getBinaryName(enclosing_element) + "";
      final ModelInjectionData injection =
          new ModelInjectionData(package_name, element, element_class
      ...
      if (model_injections.containsKey(enclosing_element)) {
        ...
        final List<ModelInjectionData> injection_list = new ArrayList<>();
        injection_list.add(injection);
        model_injections.put(enclosing_element, injection_list);
      }
    }
  }
  ...
  private static void addPresentationFragmentInjection(
      Map<Element, List<PresentationFragmentInjectionData>> presentation_fragment_injections
      ,
      Map<Element, List<PresentationFragmentInjectionData>> controller_presentation_fragment_injections
    ...
    if (enclosing_element.getAnnotation(ControllerImplementation.class) != null) {
      if (controller_presentation_fragment_injections.containsKey(enclosing_element)) {
        ...
        final List<PresentationFragmentInjectionData> injection_list = new ArrayList<>();
        injection_list.add(injection);
        controller_presentation_fragment_injections.put(enclosing_element, injection_list);
      }
      ...
      final List<PresentationFragmentInjectionData> injection_list = new ArrayList<>();
      injection_list.add(injection);
      presentation_fragment_injections.put(enclosing_element, injection_list);
    }
  }
  ...
  private void processPresentationInjections(RoundEnvironment environment,
    ...
    for (Element element : environment.getElementsAnnotatedWith(InjectPresentation.class)) {
      ...
      final String package_name = _element_utilities.getPackageOf(enclosing_element) + "";
      final String element_class = _element_utilities.getBinaryName(enclosing_element) + "";
      final PresentationInjectionData injection =
          new PresentationInjectionData(package_name, element, element_class);
  ...
  @CheckForNull
  private List<? extends VariableElement> injectModelConstructor(TypeElement element) {
    ...
    if (!found_accessible_constructor) {
      error(element, "There must be a non-private @InjectModel or default constructor (%s).",
          element);
    }
    return parameters;
  }
